...
function getDataFromSpreadsheet(sheetsID) {
  ...
  ws.forEach((sheet) => {
    if (!templateExclude.includes(sheet.getName())) {
      ...
      const fltrData = rowValues.filter(([rowValues]) => rowValues != [""])
      // console.log(fltrData);
      data = data.concat(fltrData)
...
function datatableView() {
  ...
  const dataRange = ws.getRange("A2").getDataRegion()
  const dataReg = dataRange.getDisplayValues()

  const headers = dataReg.shift()

  const jsData = dataReg.map((r) => {
    const tempObject = {}
...
function loadImageBytes() {
  var id = "14Y45ey9aET20OKvjCzu3yScyZ_MjIOh-"
